...
// USING FETCH & ASYNC AWAIT example prompt
// document
//   .getElementById("pokemonLoad")
...
document
  ...
  .addEventListener("click", async function (e) {
    ...
    let pokemonSearch = document.getElementById("pokemonName").value;
    try {
      let pokemonList = await fetch(
        // URL: for ICE -> Store user input in variable called pokemonName
        // `https://pokeapi.co/api/v2/pokemon/${pokemonName}`
      ...
      let typeString = '';
      jsonResults.types.forEach(type => {
        typeString += `${type.type.name} `;
      });
      pokemonString += `<li> <div id="pokemonCard"> <h3>Name: <a href="https://pokeapi.co/api/v2/pokemon/${pokemonSearch}"> ${jsonResults.name}</a> </h3> <br> <img src=${jsonResults.sprites.front_default}> <br> <p> <br>
      <br>
      Id: ${jsonResults.id} <br> 
      Types: ${typeString} <br> </p> </div> </li> `;
